...
void displayMap(const vector <string>& mazemap, int screenSizeX, int screenSizeY, int linepointer, int colpointer, int playerPosY, int playerPosX, const vector<pair<int, int>>& monsterPositions);
...
{   
    ...
    // Move monsters after the player makes a move
    moveMonsters(mazemap, monsterPositions, make_pair(playerPosY, playerPosX));

    displayMap(mazemap, screenSizeX, screenSizeY, linepointer, colpointer, playerPosY, playerPosX, monsterPositions);
    refresh();
...
bool isFree(const vector<string>& mazemap, int y, int x) {
    return y >= 0 && y < mazemap.size() && x >= 0 && x < mazemap[0].size() && mazemap[y][x] != '#';
}
...
void displayMap(const vector <string>& originalMazemap, int screenSizeX, int screenSizeY, int linepointer, int colpointer, int playerPosY, int playerPosX, const vector<pair<int, int>>& monsterPositions)
{
    ...
    {
        if (i >= linepointer && i < (screenSizeY + linepointer)) {
            printw((mazeStrip + "\n").c_str());
        }
        i++;
    }
